...
def audioread(path, norm=False, start=0, stop=None, target_level=-25):
    ...
    try:
        if stop is not None:
            audio, sample_rate = librosa.load(path, offset=start, duration=stop - start)
        else:
            audio, sample_rate = librosa.load(path, offset=start)
...
def audiowrite(
    ...
    if not os.path.exists(destdir):
        os.makedirs(destdir)
    destpath = destpath.replace(".opus", ".wav")
